window.onload = () => {
    ...
    search_btn.addEventListener('click', throwFirstPetition);
}
...
function throwFirstPetition(){
    ...
    request.onreadystatechange = firstResponseTreatment;
...
function throwPetition(){
    ...
    request.onreadystatechange = responseTreatment;
...
function firstResponseTreatment(){
    document.getElementById("results").innerHTML = "";
    if (request.readyState === XMLHttpRequest.DONE) {
        if (request.status === 200) {
        var response = JSON.parse(request.responseText);
